...
public class ForEach extends Task implements TaskContainer
{
	...
	/**
	 * A list of dynamic properties. dynamic properties define all parameters
	 * used in loop process
	 */
	...
	/**
	 * Add a dynamic property. Dynamic property has its own semantics, its
	 * runtime value will be different to different targets.
	 * 
	 * @param property
	 *            a new dynamic property.
	 */
	...
	public void execute( ) throws BuildException
	{
		...
		for ( int i = 0; i < list.getCount( ); i++ )
		{
			...
			{
				DynamicProperty property = (DynamicProperty) dynamicProperties
						.get( j );
			...
			if ( loggerPath != null )
			{
				...
				try
				{
					...
					str = new PrintStream( new FileOutputStream( new File(
							loggerPathDir, fileName ), true ), true ); //$NON-NLS-1$
				}
			...
			for ( Enumeration e = nestedTasks.elements( ); e.hasMoreElements( ); )
			{
				...
				{
					nestedTask.perform( );
					getProject( ).setProperty( failurePropertyName,
							TASK_STATUS_PASS ); //$NON-NLS-1$
				}
				catch ( BuildException ex )
				{

					log(
							"Execution error on task: [" + nestedTask.getTaskName( ) + "] message: " + ex.toString( ), Project.MSG_WARN ); //$NON-NLS-1$ //$NON-NLS-2$
					...
					if ( "Ant".equalsIgnoreCase( nestedTask.getTaskName( ) ) ) //$NON-NLS-1$
					{
						...
						{
							getProject( ).setProperty( failurePropertyName,
									TASK_STATUS_FAIL ); //$NON-NLS-1$
							log( "Project failed on itself: ", Project.MSG_ERR ); //$NON-NLS-1$
						}
						else
						{
							getProject( ).setProperty( failurePropertyName,
									TASK_STATUS_FAIL_ON_DEPENDENCY );
							log(
									"Project failed on dependencies:", Project.MSG_WARN ); //$NON-NLS-1$ //$NON-NLS-2$
						}
					}
	 ...
	 * <li>Other tokens will be treated as property key of current project.
	 * <br>
	 * <br>
	...
	private boolean isThisProjectError( BuildException ex )
	{
		...
		baseScriptPath = baseScriptFileName.substring( 0,
				baseScriptFileName.lastIndexOf( File.separator ) );
		...
		{
			Location extScriptLocation = ( (BuildException) old )
					.getLocation( );
			...
			if ( extScriptLocation != null )
			{
				...
				//if the path of the bottom most level build file in exception is different with the second level, means failed on dependency  
				if ( extScriptFileName != null
						&& !extScriptFileName.startsWith( baseScriptPath ) )
				{
					log(
							"Task failed on its dependency : " + extScriptLocation.getFileName( ) + " Original script path " + baseScriptPath, Project.MSG_ERR ); //$NON-NLS-1$ //$NON-NLS-2$
					return false;
				}
			}
		}
