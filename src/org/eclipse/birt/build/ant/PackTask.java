...
public class PackTask extends Task
{
		...
		try
		{
			for ( BundleItem bundleItem : bundleItems )
			{
				...
				if ( bundleFile != null )
				{
					try
					{
						log( "load bundle " + bundleName + " from "
								+ bundleFile.getName( ), Project.MSG_INFO );
					...
					{
						log( "failed to load the bundle" + bundleName, ex,
								Project.MSG_WARN );
					}
				}
				else
	...
	private static Filter DEFAULT_FILTER = new Filter( new String[]{},
			new String[]{"plugin\\..*", "fragment\\..*",
					"(.*\\.jar/)?META-INF/MANIFEST\\.MF",
					"(.*\\.jar/)?META-INF/ECLIPSEF\\..*",
					"(.*\\.jar/)?META-INF/eclipse\\.inf", "about\\..*",
					"about_files/.*"} );
