...
/**
 * 
 * this custom task implements retrail logic,
 * it's a task container.  Tasks in this container will retry
 * <code>retrial</code> times, the interval also can be set
 * following attributes can be set:
 * <br>
 * <li> <code>retrial</code> retry times, default is 1.
 * <li> <code>interval</code>the interval between two execution, the units is ms, default is 0.
 * <li><code>failOnError</code>Stop the build process if failed, default is true;
 */
public class ReliableTask extends Task implements TaskContainer
{
    ...
    public void execute() throws BuildException
    {
        ...
        while (count < retrial && !pass)
        {
            ...
            try
            {
                for (Enumeration e = nestedTasks.elements(); e
                        .hasMoreElements();)
                {
            ...
            } catch (BuildException e)
            {
                count++;
                pass = false;
                log("excute reliableTask failed one time!",
                        Project.MSG_INFO);
                try
                {
